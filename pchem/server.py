...
def run_api():
    try:
        ...
        if API_NAME_KEY in request.json:
            ...
            # Execute the API
            pchem_response = pchem.apis.run(api_name, args)
        else:
            pchem_response = pchem.utils.create_pchem_response(RESPONSE_STATUS.HTTP_REQUEST_ERROR, "Malformed request. POST JSON does not contain API name. API name should be specified as 'api_name'.")
                ...
                with open(port_allocation_path, 'r+') as ports_fp:
                    ...
                    if action == "reserve":
                        ...
                        if num_free_ports < num_requested_ports:
                            pchem_response = pchem.utils.create_pchem_response(RESPONSE_STATUS.EXECUTION_ERROR, f"Not enough free ports. (Number of free ports = {num_free_ports}) > (Number of requested ports = {num_requested_ports})")
                        else:
                            free_ports = pchem.utils.get_n_free_ports(port_allocation, num_requested_ports)
                            ...
                            pchem_response = pchem.utils.create_pchem_response(RESPONSE_STATUS.OK, "Successfully reserved ports for the radio nodes.")
                    elif action == "free":
                        ...
                        with open(port_allocation_path, 'w') as ports_fp_w:
                                ports_fp_w.write(json.dumps(port_allocation))
                        pchem_response = pchem.utils.create_pchem_response(RESPONSE_STATUS.OK, "Successfully freed ports belonging to the radio nodes.")
        else:
            pchem_response = pchem.utils.create_pchem_response(RESPONSE_STATUS.HTTP_REQUEST_ERROR, "Malformed request. POST JSON does not contain action field. Action can be 'free' or 'reserve'")
