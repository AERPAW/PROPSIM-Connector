...
# A wrapper to execute APIs
def run(api_name, args):
    pchem_response = _execute_api(api_name, args, True)
    return pchem_response

# An internal function which executes the API. Ensures that the API's validator is always called. Returns the PCHEM response.
def _execute_api(api_name, args, b_execute = False, at_command=""):
        ...
        if validation_result[IS_VALID_KEY]:
            if b_execute:
                ...
                if api_implemented:
                    ...
                    pchem_response = propsim_interface.PropsimSocket().execute_at_command(at_command) # Execute AT command using the PropSim Interface Singleton
                else:
